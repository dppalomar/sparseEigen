...
#----------#
# Recovery #
recovery_sparse <- abs(diag(t(res_sparse$vectors) %*% V[, 1:q])) # recovery
print(recovery_sparse)

if (n >= m) {
  recovery_sparseCov <- abs(diag(t(res_sparseCov$vectors[, 1:q]) %*% V[, 1:q])) # recovery
  print(recovery_sparseCov)
}

recovery_standard <- abs(diag(t(res_standard$vectors[, 1:q]) %*% V[, 1:q])) # recovery
