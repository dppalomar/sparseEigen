...
First, we illustrate how the functions scale with dimension. For this, we generate synthetic data with sparse eigenvectors (see next section for details) of increasing dimension (100 Monte Carlo runs in each dimension). We apply the two functions to extract the first three eigenvectors. The figure below illustrates how the running time of the two functions increase as we increase the dimension of the problem. It is clear that `spEigen()` scales very well, while `spca()` becomes impractical for large dimensions.
...
########## Plots ##########
png(file="lala.png", width = ???, height = ???, units = "cm", res = 120)  #default res = 72
...
![Average running time.](figures/running_time.eps){width=95%}
