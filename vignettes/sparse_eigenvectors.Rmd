...
=======
output:
>>>>>>> 191b3636aae4400d7ac143e9cb7d913c6e5904b5
...
$$w_{i}^{(k)}= \begin{cases}
		...
		\end{cases}$$
with $\mathbf{w}\in\mathbb{R}_+^{mq}$, $\mathbf{u}^{(k)} = \text{vec}(\mathbf{U}^{(k)})\in\mathbb{R}_+^{mq}$, $\mathbf{w}_{\max}\in\mathbb{R}^q_+$, with $w_{\max,i}$ being the maximum weight that corresponds to $\mathbf{u}^{(k)}_{i}$.
